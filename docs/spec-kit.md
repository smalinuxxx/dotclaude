...
**ايه الهدف؟**
لو فهمت ليه المشروع دا اتعمل من البدايه هتفهم امتى وليه تستخدمه.
الهدف من المشروع دا انه يوثق كل الخطوات بشكل عميق كأنه plan بادق التفاصيل الممكنه
ويحتفظ بالـ doc دا او بالـ md files دى على جنب فى spec dir
ويفصله عن الـ implmentation
بعدين لو عايز تمسح الـ impl وتغيره تماماً وتغير كمان الـ LLM اللى استخدمته, تقدر تعمل كدا بسهوله
لان عندك وصف دقيق لكل خطوه وتقدر تمشى اى LLM عليهم وتعمل impl من أول وجديد.

**العيوب**
1. انا ما شفتش plan mode متفعل هنا. ومعروفه ان claude بيشتغل حلو جدا لما تبدأ بـ plan mode
   فى المستقبل المفروض هعمل خمس plans مش بس plan واحده.
   الهدف ان كل الاحتمالات والـ features اللى بحلم بيها تكون موجوده فى الأول حتى لو مش هنفذها.
   كلامى هتقلب لأكتر من صيغه, وكل صيغه هيتعمل بيها plan بـ approach مختلف.
2. كل spec بيتعمل فى new branch وفى الحقيقه دا عيب, لانى هستخدم worktree مش branches !
3. 

**الميزات**
1. اخد حته الـ planing لمرحله عميقه جدا. وعملها standard
2. الادوات والاسكربتس نفسها مش بتاخد تماماً من الـ context
3. 

____
حمل الـ template بتاع claude واستخدمه مباشر, 
https://github.com/github/spec-kit/releases
ممنوع تحقن حاجه من برا مهما كانت مشهوره. يمكن بتضيف حاجات انت مش حاببها او بتعمل side effect!
وامسح الأمر دا
```shell
...
#### 1. **Create Constitution** (First time only)

Define your project's non-negotiable principles, tech stack, coding standards, and rules. This guides all future AI-generated code.

```
/speckit.constitution      


Create or update the project constitution from interactive or provided principle inputs, ensuring all dependent templates stay in sync. (project)
```
...
Describe **what** you want to build and **why** (not how). Focus on: 
- User intent and problem being solved 
...
**كل ما تحتاج تعمل feature جديده بتنفذ الأمر دا أول حاجه.**

**هنا المفروض بتركز على what & why مش على how**
المفروض هنا من تكتبش tech stack لانه ممكن يتغير, مثلا فى المستقبل ممكن تستخدم Go بدل python وهكذا.
هنا بتكتب الحاجات اللى لا يمكن تتغير, الـ main business requirements

مع نهايه الأمر دا بيتم انشاء ملف spec.md 
وملف checklists/requirements.md

الأمر دا بتحتاجه كل لما تكون عايز تعمل feature جديده
...
/speckit.clarify
```
The AI will ask questions to remove ambiguities and edge cases. Answer these to strengthen your spec.

#### 4. **Create Technical Plan**
Generates architecture, technology choices, data contracts, and implementation approach based on your spec and constitution.

```
/speckit.plan              

Execute the implementation planning workflow using the plan template to generate design artifacts. (project)
```

الخطوه دى اللى كتبو فيها تفاصيل الـ impl 
مثلا اى tech stack مطلوب, الخ

#### 5. **Generate Tasks**
Breaks down the plan into concrete, numbered tasks (T001, T002, etc.).
...
الخطوة دى بتمسك الـ plan وتقسمها وتقطعها على اجزاء 

هنا النقطه اللى ممكن تقف فيها وتنقل للـ TDD
...
النقطه اللى تكتب فيها tests لكل task وتخلى الـ LLM يبدأ يعمل impl لكل task
وهنا الخطوة اللى ممكن تنقل منها وتنفذ الـ Ralph Wiggum




وصف عميق للـ flow دا:
